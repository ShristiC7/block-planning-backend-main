...
const RailTwinEngine = (function() {
  ...
  // --- Backend API Configuration (Render live backend) ---
  const API_BASE = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
    ? (localStorage.getItem('rt_api_base') || 'https://railblock-api.onrender.com')
    : (localStorage.getItem('rt_api_base') || 'https://railblock-api.onrender.com');

  let remoteDataSynced = false;
  let backendKPIData = null;
  let backendDefects = [];
  let backendBlockPlans = [];
  let backendBlockRequests = [];
...
TSK-3001,OHE-501,TRD,Catenary Insulator High-Pressure Washing,KM 42.0 - 45.0,150,0.80,0.75,0.70,0.50,0.60,HIGH,25kV Power Block,POWER_BLOCK,Tower Wagon + 6 Fitters`;
    ...
    // --- Backend API Live Integration Suite ---
    getApiBase: () => API_BASE,
    setApiBase: (url) => {
      localStorage.setItem('rt_api_base', url);
    },
    isBackendSynced: () => remoteDataSynced,
    getBackendKPIData: () => backendKPIData,
    getBackendBlockPlans: () => backendBlockPlans,
    getBackendBlockRequests: () => backendBlockRequests,

    fetchBackendSummary: async function() {
      try {
        const res = await fetch(`${API_BASE}/summary`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        backendKPIData = data;
        setStored('backend_kpis', data);
        RailTwinEngine.logAudit("API_SYNC_SUMMARY", `Fetched live KPIs from ${API_BASE}/summary.`);
        return { success: true, data };
      } catch (err) {
        console.warn("Backend /summary fetch failed, using fallback:", err);
        return { success: false, error: err.message };
      }
    },

    fetchBackendDefects: async function(mergeIntoBacklog = false) {
      try {
        const res = await fetch(`${API_BASE}/defects`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        backendDefects = data.defects || [];
        
        if (mergeIntoBacklog && backendDefects.length > 0) {
          const mappedTasks = backendDefects.map(d => {
            const deptCode = (d.department || 'TMS').toUpperCase();
            let dept = 'ENG';
            let deptName = 'Track Engineering (TMS)';
            let blockType = 'TRAFFIC_BLOCK';
            let safety = 'Traffic Block Required';

            if (deptCode.includes('SNT') || deptCode.includes('SMMS')) {
              dept = 'SNT';
              deptName = 'Signal & Telecom (SMMS)';
              blockType = 'SIGNAL_DISCONNECTION';
              safety = 'Signal Disconnection Notice';
            } else if (deptCode.includes('TRD') || deptCode.includes('TDMS')) {
              dept = 'TRD';
              deptName = 'Traction Distribution (TDMS)';
              blockType = 'POWER_BLOCK';
              safety = '25kV AC Catenary Isolation';
            }

            const crit = (d.severity || 5) / 10;
            return {
              id: `DEF-${d.defect_id}`,
              code: `${deptCode}-${d.defect_id}`,
              dept: dept,
              deptName: deptName,
              source: `Render API (${deptCode})`,
              title: d.defect_type || `Track/Signal Defect #${d.defect_id}`,
              location: `${d.section_id || 'SEC-01'} (KM ${d.start_km || 0}-${d.end_km || 10})`,
              durationMinutes: (d.estimated_duration_hours || 2) * 60,
              criticality: crit,
              severity: crit,
              urgency: Math.min(1.0, (d.days_overdue || 1) / 30),
              opImpact: 0.7,
              failureRisk: crit,
              status: d.safety_signal ? 'CRITICAL' : ((d.severity || 0) >= 7 ? 'CRITICAL' : 'HIGH'),
              safetyProfile: safety,
              blockType: blockType,
              machineReq: 'Crew & Standard Cadre Machinery',
              priorityScore: Math.round(d.priority_score ? d.priority_score * 100 : crit * 100)
            };
          });

          tasks = [...mappedTasks, ...INITIAL_TASKS];
          setStored('tasks', tasks);
          RailTwinEngine.logAudit("API_SYNC_DEFECTS", `Ingested ${mappedTasks.length} live tasks from Render backend into task database.`);
        }
        return { success: true, count: backendDefects.length, defects: backendDefects };
      } catch (err) {
        console.warn("Backend /defects fetch failed:", err);
        return { success: false, error: err.message };
      }
    },

    fetchBackendBlockPlans: async function(horizon = null) {
      try {
        const url = horizon ? `${API_BASE}/block-plan?horizon=${horizon}` : `${API_BASE}/block-plan`;
        const res = await fetch(url);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        backendBlockPlans = data.block_plan || [];
        return { success: true, count: backendBlockPlans.length, plans: backendBlockPlans };
      } catch (err) {
        console.warn("Backend /block-plan fetch failed:", err);
        return { success: false, error: err.message };
      }
    },
    ...
    fetchBackendBlockRequests: async function() {
      try {
        ...
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        backendBlockRequests = data.block_requests || [];
        return { success: true, count: backendBlockRequests.length, requests: backendBlockRequests };
      } catch (err) {
        console.warn("Backend /block-requests fetch failed:", err);
        return { success: false, error: err.message };
      }
    },

    syncAllBackendData: async function() {
      const [sumRes, defRes, planRes, reqRes] = await Promise.allSettled([
        RailTwinEngine.fetchBackendSummary(),
        RailTwinEngine.fetchBackendDefects(true),
        RailTwinEngine.fetchBackendBlockPlans(),
        RailTwinEngine.fetchBackendBlockRequests()
      ]);
      remoteDataSynced = true;
      setStored('remote_synced', true);
      window.dispatchEvent(new CustomEvent('railtwin:backend_synced', {
        detail: { sumRes, defRes, planRes, reqRes }
      }));
      return { success: true };
    },
